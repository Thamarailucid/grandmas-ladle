...
export const findAll = async () => {
  const result = await database.query('SELECT * FROM festivals ORDER BY created_at DESC');
...
export const create = async (data: any) => {
  ...
  const query = "INSERT INTO festivals (" + fields.join(', ') + ") VALUES (" + placeholders.join(', ') + ") RETURNING *";
...
export const deleteRecord = async (id: string) => {
  await database.query('DELETE FROM festivals WHERE id = $1', [id]);
};